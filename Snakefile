...
rule main:
  input: expand("results/{sample}/06-filtering/{sample}.{pcr}.clusters.stats.tsv", sample=config["samples"], pcr=config["primers"].keys())

#Rule for quality trimming sequencing reads
rule trim:
 output:
  read1   = "results/{sample}/01-sickle-fastqc/{sample}.trimmed_read1.fastq",
  read2   = "results/{sample}/01-sickle-fastqc/{sample}.trimmed_read2.fastq",
  singles = "results/{sample}/01-sickle-fastqc/{sample}.trimmed_singles.fastq",
  log     = "results/{sample}/01-sickle-fastqc/{sample}.trimming_by_sickle.log"
 input:
 ...
 shell:
  r"""sickle pe -f {input.read1} -r {input.read2} -o {output.read1} -p {output.read2} -s {output.singles} -t sanger -q {params.qual_threshold} -l {params.len_threshold} > {output.log}
    fastqc -f fastq -o {params.folder}/01-sickle-fastqc {input.read1} {input.read2}
    fastqc -f fastq -o {params.folder}/01-sickle-fastqc {output.read1} {output.read2}
  """

#Rule to extend overlapping paired end reads
rule overlap:
 output: "results/{sample}/02-flash/{sample}.overlap_by_flash.log"
 input:
  read1 = "results/{sample}/01-sickle-fastqc/{sample}.trimmed_read1.fastq",
  read2 = "results/{sample}/01-sickle-fastqc/{sample}.trimmed_read2.fastq",
 params:
  ...
  prefix      = "{sample}"
 shell:
  r"""flash -m {params.min_overlap} -M {params.max_overlap} -O -o {params.prefix} -d {params.directory}/02-flash --threads=1 {input.read1} {input.read2} > {output}
      gzip {input.read1}
      gzip {input.read2}
      gzip results/{params.prefix}/01-sickle-fastqc/{params.prefix}.trimmed_singles.fastq
      gzip results/{params.prefix}/02-flash/{params.prefix}.notCombined_1.fastq
      gzip results/{params.prefix}/02-flash/{params.prefix}.notCombined_2.fastq
  """

#Rule to segragate extended reads using primer sequencing and the cluster 100% identical sequences
rule sort:
 output: "results/{sample}/03-sort/{sample}.{pcr}.sort.stats.tsv"
 input: "results/{sample}/02-flash/{sample}.overlap_by_flash.log"
 params:
  prefix    = "{sample}",
  directory = "results/{sample}",
  pcr       = "{pcr}",
...
#Rule to run blast 
rule blast:
 output: "results/{sample}/04-blast/{sample}.{pcr}.clusters.blast"
 input: "results/{sample}/03-sort/{sample}.{pcr}.sort.stats.tsv"
 params:
  reference = lambda wildcards: config['database'][wildcards.pcr],
  query     = "results/{sample}/03-sort/{sample}.{pcr}.clusters.fasta"
 shell:
...
rule analyse_blast:
 output: "results/{sample}/05-analyze_blast/{sample}.{pcr}.clusters.blast.stats.tsv"
 input: "results/{sample}/04-blast/{sample}.{pcr}.clusters.blast"
 params:
  prefix         = "{sample}",
  pcr            = "{pcr}",
  directory      = "results/{sample}/04-blast/",
  length         = lambda wildcards: config['amplicon_size'][wildcards.pcr],
  clusters_fasta = "results/{sample}/03-sort/{sample}.{pcr}.clusters.fasta",
 shell:
...
rule filtering:
 output: "results/{sample}/06-filtering/{sample}.{pcr}.clusters.stats.tsv"
 input: "results/{sample}/05-analyze_blast/{sample}.{pcr}.clusters.blast.stats.tsv"
 params:
  prefix    = "{sample}",
  directory = "results/{sample}",
  pcr       = "{pcr}",
