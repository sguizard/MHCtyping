...
my $selected_file_dqa   = glob("*.DQA.selected.tsv"       );
my $selected_file_dqb   = glob("*.DQB.selected.tsv"       );
my $selected_file_drb3  = glob("*.DRB3.selected.tsv"      );
my $discarded_file_dqa  = glob("*.DQA.discarded.tsv"      );
my $discarded_file_dqb  = glob("*.DQB.discarded.tsv"      );
my $discarded_file_drb3 = glob("*.DRB3.discarded.txt"     );
my $summary_file_dqa    = glob("*.summary.DQA.matrix.tsv" );
my $summary_file_dqb    = glob("*.summary.DQB.matrix.tsv" );
my $summary_file_drb3   = glob("*.summary.DRB3.matrix.tsv");
...
if (-e $samplesheet){
	...
	while(<SAMPLES>){
		...
		if ($dqa_counts{$sample} > 100 or \
			$dqb_counts{$sample} > 100 or \
			$drb3_counts{$sample} > 100){
			# print OUT_HP "$sample";
			# print OUT_HP_GENES "$sample";
